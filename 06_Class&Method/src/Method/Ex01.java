 ...
 * 					}
 * 
 * 			- 접근제한 : public > protected > default > private ( 클래스 , 메서드 , 변수 앞에 사용됨)
 			- 반환형 : - 메서드를 실행하고 특정 타입의 결과를 반환해 주겠다고 선언하는 것을 반환형(리턴타입)이라고 한다.
 					  ...
 					  - 반환형이 void인 경우를 제외하고 결과를 되돌려주어야 할 때 메서드 명령문 맨 마지막에 return이라는 키워드를 작성 후 , 앞에서 선언한
 					  	반환형과 같은 자료형으로 결과를 되돌려준다.
 			- 메서드이름 : 식별자, 소문자로 시작, 두개의 단어가 결합이 된 경우에는 두 번째 글자의 첫 글자는 대문자로 작성하는 것이 관례임. ex) void humanMan(){};
...
public class Ex01 {
	...
	public static void add() {
		System.out.println("add() 메서드호출하기!!!!");
	} // add() 메서드 end
