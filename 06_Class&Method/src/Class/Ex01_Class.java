 ...
 * 	  예를 든다면 사람은 이름 , 나이 등의 속성(멤버변수)이 있고, 걷다, 쓰다, 달린다 등의 동작(메서드)이 있음.
	...
	- 사용관계는 객체 간의 상호작용을 말함.다른 객체의 메서드를 호출하여 원하는 결과를
	  얻어내는 것을 말함.예를 든다면 사람은 자동차를 사용하므로 사람과 자동차는 사용의 관계라고 볼 수 있음.
	...
	- 객체지향 프로그래밍 (OOP : Object Oriented Programming)은 만들고자
	  하는 완성품인 객체를 모델링하고, 집합관계에 있는 부품객체와 사용관계에 있는 객체를
	  하나씩 설계한 후 조립하는 방식으로 프로그램을 개발하는 기법임.
 */

/*
 * 클래스(Class)?
 * - 현실에서 객체는 갑자기 만들어지는것이 아니라, 설계도를 바탕으로 만들어짐.
...
public class Ex01_Class { // 클래스 헤더
	...
	public static void main(String[] args) {
		...
		// 		형식) 참조변수 = new 클래스 이름();  ==> 클래스이름(x), 생성장(o)
		//ex = new Ex01_Class();  => 객체를 생성
		
		
		// 1단계 + 2단계가 같이 이루어짐 : 클래스 선언 및 클래스 객체 생성.
		...
		// Ex01_Class 객체에 인스턴스화하여 새로운 주소값을 만듬 0x200의 공간
